...
extern "C"
{
    int SIMPLX_SDK_COMPATIBILITY_VERSION = simplx::Engine::SDK_COMPATIBILITY_VERSION;
    int SIMPLX_SDK_PATCH_VERSION = simplx::Engine::SDK_PATCH_VERSION;
    int SIMPLX_SDK_COMPILER_ID = SIMPLX_COMPILER_ID;
}
...
{
    std::stringstream ss;
    ss << "Simplx C++ Simplx framework" << (int)SIMPLX_ENGINE_VERSION_MAJOR << "." << (int)SIMPLX_ENGINE_VERSION_MINOR;
    if (SIMPLX_ENGINE_VERSION_PATCH != 0)
    {
        ss << '.' << (int)SIMPLX_ENGINE_VERSION_PATCH;
    }
    ss << " "
#ifndef NDEBUG
       << "debug "
       ...
       << "windows "
#endif
#if SIMPLX_COMPILER_ID == 1
       << "gcc" << (int)__GNUC__ << '.' << (int)__GNUC_MINOR__
#else
       << "COMPILER=" << (int)SIMPLX_COMPILER_ID
#endif
       << ' ';
...
void Engine::start(const StartSequence &startSequence)
{
         ...
         i != endi; ++i)
    {
        ...
        try
        {
            ...
            if (i->isServiceFlag)
            {
                assert(serviceDestroyActorId != simplx::null);
