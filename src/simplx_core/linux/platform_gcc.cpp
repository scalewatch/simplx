 ...
 */
vector<string> simplx::debugBacktrace(const uint8_t stackTraceSize) noexcept
{
 ...
 */
string simplx::demangleFromSymbolName(char *symbolName) noexcept
{
...
vector<string> simplx::debugBacktrace(const uint8_t) noexcept
{
    return {""};
}
...
string simplx::cppDemangledTypeInfoName(const type_info &typeInfo)
{
    int status = 0;
...
simplx::thread_t simplx::threadCreate(void (*fn)(void *), void *param, size_t stackSizeBytes)
{
    struct Callback
    {
        void (*fn)(void *);
...
void simplx::threadSetAffinity(unsigned cpuIndex)
{
    cpuset_type cpuset;
...
void simplx::threadSetAffinity(const cpuset_type &pcpuset)
{
    int cc;
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
...
simplx::cpuset_type simplx::threadGetAffinity(void)
{
    int cc;
    cpu_set_t cpuset;
...
void simplx::threadSetRealTime(bool rt, const ThreadRealTimeParam &param)
{
    int cc;
...
void simplx::threadSleep(const Time &delay)
{
...
void simplx::tlsDestroy(tls_t key)
{
...
//---- Mutex CLASS (not primitive) DTOR ----------------------------------------
    ...
    simplx::Mutex::~Mutex() noexcept
{
#ifndef NDEBUG
    SIMPLX_TRY
    assert(!debugIsLocked());
    mutexDestroy(handle); // throw(simplx::RunTimeException)
    
    SIMPLX_CATCH_AND_EXIT_FAILURE_WITH_CERR_MESSAGE
#endif
}
