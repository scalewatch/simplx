...
#include "simplx_core/internal/service.h"

extern "C"
{

#ifdef SIMPLX_COMPILER_ID
    #error SIMPLX_COMPILER_ID macro already defined
#endif
    #pragma pack(push, 1)
#if defined(__GNUG__)
    /**
     * @def SIMPLX_COMPILER_ID
     * @brief Compiler specific identifier used for library compatibility check
     */
    #define SIMPLX_COMPILER_ID 1
    struct SIMPLX_SDK_COMPILER_VERSION_type
    {
        int vmajor;
        int vminor;
        int vpatch;
        inline SIMPLX_SDK_COMPILER_VERSION_type() : vmajor(__GNUC__), vminor(__GNUC_MINOR__), vpatch(__GNUC_PATCHLEVEL__)
        {
        }
    };
#else
...
public:
    ...
    static const int SDK_COMPATIBILITY_VERSION = SIMPLX_ENGINE_VERSION_MINOR; /**< SDK minor version */
    static const int SDK_PATCH_VERSION = SIMPLX_ENGINE_VERSION_PATCH;         /**< SDK patch version */
    static const int DEFAULT_EVENT_ALLOCATOR_PAGE_SIZE = 64 * 1024; /**< Default event allocator page size */

    typedef Actor::CoreId CoreId;
    /**
     * @brief Runtime exception that can be thrown when checking the user's binary compatibility with the Simplx
     * library
     */
    struct RuntimeCompatibilityException : std::exception
    {
        ...
        virtual const char *what() const noexcept
        {
            ...
            case E_DEBUG_RELEASE:
                return "simplx::Engine::RuntimeCompatibilityException [debug/release mode used to build current "
                       "binary mismatches SIMPLX RUNTIME]";
            case E_COMPILER_VERSION:
                return "simplx::Engine::RuntimeCompatibilityException [inconsistent version of C++ compiler "
                       "used to build SIMPLX SDK and the current binary]";
            case E_ENGINE_VERSION:
                return "simplx::Engine::RuntimeCompatibilityException [the SIMPLX SDK version used to build "
                       "the current binary is incompatible with the SIMPLX RUNTIME used to run the current binary]";
            default:
                return "simplx::Engine::RuntimeCompatibilityException [?]";
            }
        }
    };
    /**
    ...
    {
        virtual const char *what() const noexcept { return "simplx::Engine::CoreInUseException"; }
    };
    /**
    ...
    {
        virtual const char *what() const noexcept { return "simplx::Engine::UndersizedException"; }
    };
    ...
    class CoreSet
    {
      public:
        ...
        struct TooManyCoresException : std::exception
        {
            virtual const char *what() const noexcept
            {
                return "simplx::Engine::CoreSet::TooManyCoresException";
            }
        };
        ...
        struct UndefinedCoreException : std::exception
        {
            virtual const char *what() const noexcept
            {
                return "simplx::Engine::CoreSet::UndefinedCoreException";
            }
        };
      ...
      public:
        ...
        const Actor::ActorId &getServiceActorId() const noexcept
        {
            ...
            for (const unsigned index = getIndexValue<_Service>();
                 i < MAX_SIZE && table[i].actorId != simplx::null && table[i].index != index; ++i)
            {
            }
            if (i < MAX_SIZE)
            {
        ...
        void setServiceActorId(const Actor::ActorId &actorId)
        {
            ...
            for (; i < MAX_SIZE && table[i].actorId != simplx::null && table[i].index != index; ++i)
            {
            }
            if (i == MAX_SIZE)
            {
    ...
    class StartSequence
    {
      public:
        ...
        struct DuplicateServiceException : std::exception
        {
            virtual const char *what() const noexcept
            {
                return "simplx::Engine::StartSequence::DuplicateServiceException";
            }
        };
        /**
         ...
         * public:
         *     typedef std::vector<int, simplx::Actor::Allocator<int> > IntVector;
        ...
        /**
         * @brief Set the default thread stack size that will be used by the threads created by Simplx.
         * @param size in bytes
         */
...
private:
    ...
    public:
        ...
        {
            if (previousServiceDestroyActorId != simplx::null)
            {
                try
                {
...
void Engine::StartSequence::addServiceActor(CoreId coreId, const _ActorInit &actorInit)
{
    struct AddServiceActorStarter : ActorStarter<_ActorInit>
    {
                                            ...
                                            bool isLastService) const
        {
            ...
            {
                assert(engine.serviceIndex.getServiceActorId<_Service>() == simplx::null);
