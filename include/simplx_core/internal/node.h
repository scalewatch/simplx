/**
 * @file node.h
 * @brief simplx engine graph node
...
#include "simplx_core/internal/parallel.h"
...
#pragma pack(1)
    struct Shared
    {
        ...
        char cacheLinePadding1[SIMPLX_CACHE_LINE_PADDING(sizeof(sig_atomic_t) + sizeof(bool) + sizeof(WriteCache))];
        ReadWriteLocked readWriteLocked;
        char cacheLinePadding2[SIMPLX_CACHE_LINE_PADDING(sizeof(ReadWriteLocked))];
    ...
    struct ReaderSharedHandle
    {
        ...
        char cacheLinePadding[SIMPLX_CACHE_LINE_PADDING(sizeof(CacheLine1))];
...
class AsyncNodeAllocator
{
  public:
    ...
    AsyncNodeAllocator()
        : // throw (std::bad_alloc)
          blockChainArray((assert(sizeof(void *) <= 8),
                           static_cast<BlockChain *>(alignMalloc(simplx::CACHE_LINE_SIZE, sizeof(BlockChain) * 64))))
#ifndef NDEBUG
          ,
...
#endif
    {
        ...
        assert((uintptr_t)blockChainArray % simplx::CACHE_LINE_SIZE == 0);
    ...
    ~AsyncNodeAllocator() noexcept
    {
        ...
        if (!debugCheckMap.empty())
        {
            ...
            for (DebugCheckMap::iterator i = debugCheckMap.begin(), endi = debugCheckMap.end(); i != endi; ++i)
            {
                ...
                {
                    std::cout << "backtrace disabled (use simplx::Engine::debugActivateMemoryLeakBacktrace())"
                              << std::endl;
                }
            }
        }
        ...
        {
            simplx::alignFree(simplx::CACHE_LINE_SIZE, pageChain.pop_front());
        }
        alignFree(simplx::CACHE_LINE_SIZE, blockChainArray);
    }
#ifndef NDEBUG
    ...
    inline void *allocate(size_t sz, const void * = 0)
    {
        ...
        if (blockChain.empty())
        {
            ...
            assert(pageSz % simplx::CACHE_LINE_SIZE == 0);
            pageSz *= ((blockSz + sizeof(Block) + pageSz - 1) / pageSz);
            Block *page = static_cast<Block *>(alignMalloc(simplx::CACHE_LINE_SIZE, pageSz));
            if (page == 0)
            {
                throw std::bad_alloc();
            }
            assert((uintptr_t)page % simplx::CACHE_LINE_SIZE == 0);
#ifndef NDEBUG
    ...
    inline static unsigned index(size_t psz) noexcept
    {
        ...
        unsigned hbit = simplx::highestBit(sz);
        unsigned dbit = hbit - simplx::lowestBit(sz);
...
#pragma pack(pop)
    ...
    private:
        ...
        struct Chain : DoubleChain<0u, Chain>
        {
            ...
            getItem(simplx::MultiDoubleChainLink<NodeActorCountListener> *link) noexcept
            {
                return static_cast<NodeActorCountListener *>(link);
            }
            inline static const NodeActorCountListener *
            getItem(const simplx::MultiDoubleChainLink<NodeActorCountListener> *link) noexcept
            {
                return static_cast<const NodeActorCountListener *>(link);
            }
            inline static simplx::MultiDoubleChainLink<NodeActorCountListener> *
            getLink(NodeActorCountListener *item) noexcept
            {
                return static_cast<simplx::MultiDoubleChainLink<NodeActorCountListener> *>(item);
            }
            inline static const simplx::MultiDoubleChainLink<NodeActorCountListener> *
            getLink(const NodeActorCountListener *item) noexcept
            {
                return static_cast<const simplx::MultiDoubleChainLink<NodeActorCountListener> *>(item);
            }
        };
