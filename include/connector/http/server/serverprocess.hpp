...
#include "connector/http/server/server.hpp"
...
class HttpServerProcess : public TcpClient<_Network, _SendBufferSize, _ReceiveBufferSize>
{
    ...
    string getPath(const string &httpHeader)
    {
        ...
        if ((pathBegin - 1) == string::npos)
            return "";
        size_t pathEnd = min(httpHeader.find('?', pathBegin),httpHeader.find(' ', pathBegin));
    ...
    /**
     * @brief Get Param requested by client
     * 
     * @param httpHeader header of http request
     * @return string 
     */
    string getParam(const string &httpHeader)
    {
        size_t pathBegin = httpHeader.find(' ') + 1;
        if ((pathBegin - 1) == string::npos)
            return "";
        ...
        if (paramBegin != string::npos && pathEnd != string::npos && paramBegin < pathEnd)
        {
            size_t pathEnd = httpHeader.find(' ', paramBegin);
            if (pathEnd == string::npos)
                return "";
            return httpHeader.substr(paramBegin, pathEnd - paramBegin);
        }
        return "";
    }
     ...
     * @return vector<string> of partial path
     */
    vector<string> getParsedParam(string path)
    {
        vector<string> nodes;
        size_t index = path.find('?', 0);
    ...
    */
    ...
    void sendDocument(const string &documentPath)
    {
        ...
        try
        {
            if (documentPath.find(".css") == documentPath.length()-4) {//!= std::string::npos) {
                sendHeader(200, "text/css", len);
            }
            else if (documentPath.find(".svg") == documentPath.length()-4){
                sendHeader(200, "image/svg+xml", len);
            }
            else if (documentPath.find(".ico") == documentPath.length()-4) {
                sendHeader(200, "image/icon", len);
            }
            else if (documentPath.find(".json") == documentPath.length()-4) {
                sendHeader(200, "application/json", len);
            }
            else {
