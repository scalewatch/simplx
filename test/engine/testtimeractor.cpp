...
#include "pattern/timer/timerproxy.h"
#include "pattern/timer/timeractor.h"

using namespace simplx;
using namespace simplx::timer;
...
{
	TimerEventTestActor() : TimerProxy(static_cast<simplx::Actor&>(*this)), count(0) {
		std::cout << "TimerEventTestActor::TimerEventTestActor()" << std::endl;
		setRepeat(simplx::Time::Millisecond(100));
	}
