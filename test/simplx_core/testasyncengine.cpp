...
{
    TestStartSequence(const simplx::Engine::CoreSet &coreSet = TestCoreSet()) : Engine::StartSequence(coreSet) {}
};
...
{
    TestStartSequence startSequence;
    ASSERT_THROW(startSequence.addActor<TestActor>((simplx::Engine::CoreId)simplx::cpuGetCount()), std::runtime_error);
}
