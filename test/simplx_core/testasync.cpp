...
{
template <>
struct simplx::EngineCustomEventLoopFactory::EventLoopAutoPointerNew<TestEventLoop>
    : simplx::EngineCustomEventLoopFactory::DefaultEventLoop
{
...
struct TestEventLoop : simplx::EngineCustomEventLoopFactory::EventLoopAutoPointerNew<TestEventLoop>
{
    TestEventLoop(simplx::AsyncNode &asyncNode) : EventLoopAutoPointerNew(asyncNode) {}
};

struct TestCoreSet : simplx::Engine::CoreSet
{
    TestCoreSet()
    {
        for (int i = 0, coreCount = std::min(16, (int)simplx::cpuGetCount()); i < coreCount; ++i)
        {
            set((simplx::Engine::CoreId)i);
        }
    }
};

class TestInitActor : public simplx::Actor
{
public:
...
{
    simplx::EngineCustomEventLoopFactory customEventLoopFactory;
    simplx::AsyncNodeManager nodeManager(1024, TestCoreSet());
    simplx::AsyncNode node(simplx::AsyncNode::Init(nodeManager, 0, customEventLoopFactory));
    {
        TestInitActor &handler1 = node.newActor<TestInitActor>(0);
        simplx::Actor::ActorId handler1Id = handler1.getActorId();
        ASSERT_EQ(handler1Id.getCoreIndex(), node.id);
        ASSERT_EQ(1u, handler1Id.getNodeActorId());
        ASSERT_EQ(handler1Id, handler1Id);
        simplx::Actor::ActorId handler2Id = handler1.newUnreferencedActor<TestInitActor>(0);
...
struct TestInitRegisterEventHandler : simplx::Actor
{
...
void testUniNodeEvent()
{
    ...
    simplx::AsyncNode node(simplx::AsyncNode::Init(nodeManager, 0, customEventLoopFactory));
    {
        ...
        simplx::Actor::Event::Pipe eventPipe(handler, handler);
        ...
        ASSERT_THROW(simplx::Actor::Event::BufferedPipe(eventPipe).push<TestUniNodeEvent::Event3>(),
                     TestUniNodeEvent::Exception);
        ASSERT_THROW(simplx::Actor::Event::BufferedPipe(handler, eventPipe.getDestinationActorId())
...
void testMultinodeEvent()
{
    simplx::EngineCustomEventLoopFactory customEventLoopFactory;
    simplx::AsyncNodeManager nodeManager(1024, TestCoreSet());
    simplx::AsyncNode node1(simplx::AsyncNode::Init(nodeManager, 0, customEventLoopFactory));
    simplx::AsyncNode node2(simplx::AsyncNode::Init(nodeManager, 1, customEventLoopFactory));
    {
        ...
        simplx::Actor::Event::Pipe(handler1, handler2).push<TestMultiNodeEvent::HFEvent>();
        node1.synchronize();
        node2.synchronize();
        ASSERT_EQ(0u, handler1.getHFEventCount());
        ASSERT_EQ(0u, handler1.getLFEventCount());
        ...
        ASSERT_EQ(0u, handler2.getLFEventCount());
        ASSERT_EQ(0u, handler2.getUndeliveredEventCount());
        simplx::Actor::Event::Pipe(handler1, handler2).push<TestMultiNodeEvent::LFEvent>();
        node1.synchronize();
        node2.synchronize();
        ASSERT_EQ(0u, handler1.getHFEventCount());
        ASSERT_EQ(0u, handler1.getLFEventCount());
        ...
        ASSERT_EQ(0u, handler2.getUndeliveredEventCount());
        simplx::Actor::Event::Pipe(handler1, handler2).push<TestMultiNodeEvent::UndeliveredEvent>();
        node1.synchronize();
        node2.synchronize();
        node1.synchronize();
        ASSERT_EQ(0u, handler1.getHFEventCount());
...
static void testActorReference()
{
    ...
    simplx::AsyncNode node(simplx::AsyncNode::Init(nodeManager, 0, customEventLoopFactory));
    {
        ...
        {
            TestInitActor &handler1 = node.newActor<TestInitActor>(0);
            simplx::Actor::ActorReference<TestActorReference> handler2 =
...
{
struct SelfReferenceActor : simplx::Actor
{
    SelfReferenceActor(int) { newReferencedSingletonActor<SelfReferenceActor>(0); }
};
struct ReferenceStaticTestInitActor : simplx::Actor
{
    simplx::Actor::ActorReference<TestInitActor> ref;
...
void testStaticActorCircularReference()
{
    ...
    simplx::AsyncNode node(simplx::AsyncNode::Init(nodeManager, 0, customEventLoopFactory));
    {
        node.newActor<simplx::Actor>();

        ASSERT_THROW(node.newActor<TestStaticActorReference::SelfReferenceActor>(0), simplx::Actor::CircularReferenceException);

        TestInitActor &handler = node.newActor<TestInitActor>(0);
        simplx::Actor::ActorReference<TestInitActor> staticActor = handler.newReferencedSingletonActor<TestInitActor>(0);
        simplx::Actor::ActorReference<TestInitActor> staticActor2 = handler.newReferencedSingletonActor<TestInitActor>(0);
        ASSERT_NE(&handler, &*staticActor);
        ASSERT_EQ(&*staticActor, &*staticActor2);

        simplx::Actor::ActorReference<TestInitActor> handler2 = staticActor->newReferencedActor<TestInitActor>(0);
        
        ASSERT_THROW(handler2->newReferencedSingletonActor<TestInitActor>(0), simplx::Actor::CircularReferenceException);
        ASSERT_THROW(handler2->newReferencedActor<TestStaticActorReference::ReferenceStaticTestInitActor>(0), simplx::Actor::CircularReferenceException);
        ASSERT_THROW(handler2->newReferencedSingletonActor<TestStaticActorReference::ReferenceStaticTestInitActor>(0), simplx::Actor::CircularReferenceException);
...
struct TestOnEventException : simplx::AsyncExceptionHandler
{
    unsigned counter;
    std::string asyncActorClassName;
    const char *onXXX_FunctionName;
    const simplx::Actor::Event *event;
    const char *whatException;
    TestOnEventException() : counter(0), onXXX_FunctionName(0), event(0), whatException(0) {}
    
    void onEventException(simplx::Actor *, const std::type_info &asyncActorTypeInfo,
                                  const char *onXXX_FunctionName, const simplx::Actor::Event &event,
                                  const char *whatException) noexcept override
    {
        std::cout << "TestOnEventException::onEventException(), "
                  << (asyncActorClassName = simplx::cppDemangledTypeInfoName(asyncActorTypeInfo))
...
void testOnEventException()
{
    ...
    simplx::AsyncNodeManager nodeManager(testOnEventException, 1024, TestCoreSet());
    simplx::AsyncNode node(simplx::AsyncNode::Init(nodeManager, 0, customEventLoopFactory));
    {
        ...
        TestOnEventExceptionActor::ExceptionEvent &event =
            simplx::Actor::Event::Pipe(handler, handler).push<TestOnEventExceptionActor::ExceptionEvent>();
        node.synchronize();
        ASSERT_EQ(1u, testOnEventException.counter);
        ASSERT_EQ(testOnEventException.asyncActorClassName, simplx::cppDemangledTypeInfoName(typeid(handler)));
        ...
        TestOnEventExceptionActor::ReturnToSenderEvent &event =
            simplx::Actor::Event::Pipe(handler, handler).push<TestOnEventExceptionActor::ReturnToSenderEvent>();
        node.synchronize();
        ...
        simplx::AsyncNode node2(simplx::AsyncNode::Init(nodeManager, 1, customEventLoopFactory));
        {
            ...
            TestOnEventExceptionActor::ReturnToSenderEvent &event =
                simplx::Actor::Event::Pipe(handler, handler2)
...
void testNoDestinationPipe()
{
    ...
    simplx::EngineCustomEventLoopFactory customEventLoopFactory;
    simplx::AsyncNodeManager nodeManager(1024, TestCoreSet());
    simplx::AsyncNode node(simplx::AsyncNode::Init(nodeManager, 1, customEventLoopFactory));
    {
