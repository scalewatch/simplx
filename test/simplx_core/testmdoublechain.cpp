...
{
  private:
    struct Value : private simplx::MultiDoubleChainLink<Value, _CHAIN_COUNT>
    {
        struct Chain : simplx::MultiDoubleChainLink<Value, _CHAIN_COUNT>::template DoubleChain<_CHAIN_ID, Chain>
        {
            inline static Value *getItem(simplx::MultiDoubleChainLink<Value, _CHAIN_COUNT> *link) noexcept
            {
                return static_cast<Value *>(link);
            }
            inline static const Value *getItem(const simplx::MultiDoubleChainLink<Value, _CHAIN_COUNT> *link) noexcept
            {
                return static_cast<const Value *>(link);
            }
            inline static simplx::MultiDoubleChainLink<Value, _CHAIN_COUNT> *getLink(Value *item) noexcept
            {
                return static_cast<simplx::MultiDoubleChainLink<Value, _CHAIN_COUNT> *>(item);
            }
            inline static const simplx::MultiDoubleChainLink<Value, _CHAIN_COUNT> *getLink(const Value *item) noexcept
            {
                return static_cast<const simplx::MultiDoubleChainLink<Value, _CHAIN_COUNT> *>(item);
            }
        };
        int v;
    };
...
struct TestDoubleChain2 : simplx::MultiDoubleChainLink<TestDoubleChain2, 2>
{
    int v;
