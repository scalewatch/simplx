...
using namespace simplx::connector::tcp;
...
constexpr int64_t longTimeoutMs    = 300; // 3000 ms => 3 sec
constexpr int64_t shortTimeoutMs   = 3;   // 3 ms
constexpr bool    shouldTimeOut    = true;

TEST(tcp_clientserver, ConnectionSuccess)
{
    runTest<simplx::connector::tcp::ConnectionSuccess::ServerActor,
            simplx::connector::tcp::ConnectionSuccess::ClientActor>(longTimeoutMs);
}

TEST(tcp_clientserver, MessagePassing)
{
    runTest<simplx::connector::tcp::MessagePassing::ServerActor,
            simplx::connector::tcp::MessagePassing::ClientActor>(shortTimeoutMs, shouldTimeOut);
}
